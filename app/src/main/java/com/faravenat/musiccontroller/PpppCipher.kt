package com.faravenat.musiccontroller

object PpppCipher {
    private val KEY_TABLE = intArrayOf(
        
    )
    private val KEY = intArrayOf(0x69, 0x97, 0xCC, 0x19)

    // Encode: prevByte is the already-encoded byte (output feeds back)
    fun encode(data: ByteArray): ByteArray {
        val out = data.copyOf()
        var prev = 0
        for (i in out.indices) {
            val idx = (KEY[prev and 0x03] + prev) and 0xFF
            out[i] = (out[i].toInt() and 0xFF xor KEY_TABLE[idx]).toByte()
            prev = out[i].toInt() and 0xFF
        }
        return out
    }

    // Decode: prevByte is the raw (encoded) byte from the wire
    fun decode(data: ByteArray): ByteArray {
        val out = data.copyOf()
        var prev = 0
        for (i in out.indices) {
            val idx = (KEY[prev and 0x03] + prev) and 0xFF
            val raw = out[i].toInt() and 0xFF
            out[i] = (raw xor KEY_TABLE[idx]).toByte()
            prev = raw
        }
        return out
    }

    // Decode manteniendo estado entre paquetes: devuelve (datos_descifrados, nuevo_prev)
    fun decodeBlock(buf: ByteArray, start: Int, length: Int, prevIn: Int): Pair<ByteArray, Int> {
        var prev = prevIn
        val out = ByteArray(length)
        for (i in 0 until length) {
            val raw = buf[start + i].toInt() and 0xFF
            val idx = (KEY[prev and 0x03] + prev) and 0xFF
            out[i] = (raw xor KEY_TABLE[idx]).toByte()
            prev = raw
        }
        return out to prev
    }
}
